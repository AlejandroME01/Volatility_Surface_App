...
with st.sidebar:
    with st.expander("Model Parameters", expanded=True):
        ...
        dividend_yield = st.number_input(
            'Dividend Yield (e.g., 0.00 for no yield)',
            value=0.000,
            format="%.4f"
        )
...
if plot_type in ['2D Heatmap', 'Both']:
    ...
    heatmap_fig.update_layout(
        title=f'Implied Volatility Heatmap for {ticker_symbol}',
        xaxis_title='Time to Expiration (years)',
        yaxis_title=y_axis_option,
        width=900,
        height=600
...
st.markdown(
    """
    Created by Alejandro Morales  |  [LinkedIn](https://www.linkedin.com/in/alejandro-morales-e/)  
